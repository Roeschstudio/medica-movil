...
export class VideoCallService {
  ...
  private async retryConnection(
    ...
    console.log(
      `Retrying connection in ${delay}ms (attempt ${attempt}/${maxAttempts})`
    );
  ...
  private async flushSignalBatch(): Promise<void> {
    ...
    for (const signal of batch) {
      try {
        ...
        console.error("Failed to send batched signal:", error);
      }
    }
  }
  ...
  private async getDetailedWebRTCStats(): Promise<{
    ...
    try {
      ...
      console.error("Failed to get detailed WebRTC stats:", error);
      return { audio: {}, video: {} };
    }
  }
  ...
  private setupRealtimeSubscriptions(): void {
    if (!this.userId) {
      console.warn("Cannot setup subscriptions without user ID");
      return;
    }
