...
export class PaymentErrorHandler {
  ...
  static async retryWithBackoff<T>(
    ...
    for (let attempt = 0; attempt < attempts; attempt++) {
      try {
        ...
        console.log(`Retry attempt ${attempt + 1}/${attempts} failed:`, err);
      }
    }
  ...
  private static async logError(
    ...
    console[logLevel](`Payment Error [${error.category}/${error.severity}]:`, {
      code: error.code,
  ...
  private static async notifyAdmin(
    ...
    try {
      ...
      console.error("Failed to store admin notification:", dbError);
    }
  }
  ...
  static async getErrorStats(hours: number = 24): Promise<{
    ...
    try {
      ...
      console.error("Failed to get error stats:", dbError);
      return {
        total: 0,
        byProvider: {},
        byCategory: {},
