...
export async function GET(request: NextRequest) {
  try {
    ...
    // Feature flags API error logged internally
    return NextResponse.json(
  ...
  try {
    ...
    switch (action) {
      ...
      case "update_rollout":
        try {
          ...
          return NextResponse.json(
            {
              error:
                _error instanceof Error ? _error.message : "Invalid percentage",
            },
            { status: 400 }
          );
        }
      ...
      case "gradual_rollout":
        try {
          ...
          FeatureFlags.gradualRollout(
            ...
            .catch((_error) => {
              // Gradual rollout failed - error handled internally
            });
      ...
      case "provider_rollback":
        ...
        switch (operation) {
          case "disable":
            rollbackProcedure.disable();
            break;
          case "enable":
            rollbackProcedure.enable();
            break;
          case "emergency_disable":
            rollbackProcedure.emergencyDisable(
    ...
    // Feature flags action error logged internally
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 }
    );
...
async function logFeatureFlagChange(
  ...
  try {
    ...
    // Failed to log feature flag change - error handled internally
  }
}
