...
export class WebSocketManager {
  ...
  async connect(): Promise<void> {
    if (this.socket?.connected) {
      ErrorLogger.log(new Error("WebSocket already connected"), { context: "websocket_connect" });
      return;
    }
    ...
    try {
      ...
      ErrorLogger.log(new Error("WebSocket connected successfully"), { context: "websocket_connect", level: "info" });
    } catch (error) {
  ...
  disconnect(): void {
    ...
    ErrorLogger.log(new Error("WebSocket disconnected"), { context: "websocket_disconnect", level: "info" });
  }
  ...
  emit(event: string, data?: any): boolean {
    if (!this.socket?.connected) {
      ErrorLogger.log(new Error("Cannot emit: WebSocket not connected"), { context: "websocket_emit", event });
      return false;
    }
  ...
  private setupEventHandlers(): void {
    ...
    this.socket.on("disconnect", (reason) => {
      ...
      ErrorLogger.log(new Error(`WebSocket disconnected: ${reason}`), { context: "websocket_disconnect", reason });
  ...
  private scheduleReconnect(): void {
    if (this.stats.reconnectAttempts >= this.config.maxReconnectAttempts) {
      ErrorLogger.log(new Error("Max reconnection attempts reached"), { context: "websocket_reconnect", attempts: this.stats.reconnectAttempts });
    ...
    ErrorLogger.log(new Error(`Reconnecting in ${delay}ms (attempt ${this.stats.reconnectAttempts})`), { context: "websocket_reconnect", delay, attempt: this.stats.reconnectAttempts, level: "info" });
    ...
    this.reconnectTimeout = setTimeout(async () => {
      try {
        ...
        ErrorLogger.log(error as Error, { context: "websocket_reconnect_failed" });
        this.scheduleReconnect();
      }
    }, delay);
  }
  ...
  private startHeartbeat(): void {
    ...
    this.heartbeatInterval = setInterval(() => {
      if (this.socket?.connected) {
        ...
        ErrorLogger.log(new Error("Heartbeat: Socket not connected"), { context: "websocket_heartbeat" });
  ...
  private setState(newState: ConnectionState): void {
    if (this.state !== newState) {
      ...
      this.state = newState;
      ErrorLogger.log(new Error(`WebSocket state changed: ${oldState} -> ${newState}`), { context: "websocket_state_change", oldState, newState, level: "info" });
    }
  }
